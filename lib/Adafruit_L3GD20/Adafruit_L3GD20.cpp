...
void Adafruit_L3GD20_Unified::write8(byte reg, byte value) {
	TinyWireM.beginTransmission(L3GD20_ADDRESS);
...
byte Adafruit_L3GD20_Unified::read8(byte reg) {
	...
	TinyWireM.beginTransmission((byte)L3GD20_ADDRESS);
	TinyWireM.write((uint8_t)reg);
	TinyWireM.endTransmission();
	TinyWireM.requestFrom((byte)L3GD20_ADDRESS, (byte)1);
...
bool Adafruit_L3GD20_Unified::getEvent(sensors_event_t* event){
	...
	while(!readingValid)
	{
		...
		TinyWireM.beginTransmission((byte)L3GD20_ADDRESS);
		#if ARDUINO >= 100
			TinyWireM.write(GYRO_REGISTER_OUT_X_L | 0x80);
		#else
			TinyWireM.send(GYRO_REGISTER_OUT_X_L | 0x80);
		#endif
		if (TinyWireM.endTransmission() != 0) {
				// Error. Retry.
				continue;
		}
		TinyWireM.requestFrom((byte)L3GD20_ADDRESS, (byte)6);
		...
		#else
			uint8_t xlo = TinyWireM.receive();
			uint8_t xhi = TinyWireM.receive();
			uint8_t ylo = TinyWireM.receive();
			uint8_t yhi = TinyWireM.receive();
			uint8_t zlo = TinyWireM.receive();
			uint8_t zhi = TinyWireM.receive();
		#endif
